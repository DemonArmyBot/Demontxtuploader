...
async def txt_handler(bot: Client, m: Message):
    ...
    count = int(raw_text)    
    arg = int(raw_text)
    uploaded_count = 0  # Track uploads for cleanup every 2
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            try:
                ...
                # === CLEANUP EVERY 2 UPLOADS ===
                if uploaded_count % 2 == 0:
                    logging.info(f"[CLEANUP] Uploaded {uploaded_count} files. Clearing temp...")
                    safe_remove("thumb.jpg")
                    for ext in ["mp4", "mkv", "pdf", "html", "zip", "jpg", "png", "mp3", "wav", "m4a"]:
                        for file in os.listdir("."):
                            if file.startswith(name1[:50]) and file.endswith(f".{ext}"):
                                safe_remove(file)
                            if ".part" in file:
                                safe_remove(file)
